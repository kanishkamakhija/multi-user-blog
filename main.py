...
def make_hash_pw(name, pw, salt = None):
    if not salt:
        salt = make_salt()
    hh = hashlib.sha256(name + pw + salt).hexdigest()
    return "%s|%s" % (salt, hh)
...
class NewPost(BlogHandler):
    ...
    def post(self):
        ...
        if title and content:
            p = Post(parent = blog_key(), title = title, content = content, u_id = uid)
            p.put()
            self.redirect('/post/%s' % str(p.key().id()))
        else:
            error = "title and content are nessecary!"
            self.render("newpost.html" , title = title, content = content, error = error, u_id = uid)
...
class DeletePost(BlogHandler):
        def get(self , post_id):
            ...
            self.render('delete.html', err = err, p = post)
